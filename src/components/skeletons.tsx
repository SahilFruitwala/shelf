...
/** `/notes` — encrypted note list (also covers the decrypt pass). */
export function NotesListSkeleton({
  ...
  return (
    <SkeletonScreen label={label} className="block">
...
/** Just the note rows — used while the list is decrypting in place. */
export function NoteRowsSkeleton({ count = 5 }: { count?: number }) {
  return (
    <ul className="space-y-2">
      {Array.from({ length: count }, (_, i) => (
        <li
          key={i}
          className="rounded-(--radius-card) border border-line bg-card px-4 py-3"
        >
          <Skeleton className="h-4 w-1/2" />
...
/** `/notes/$noteId` — title, toolbar, and body of the editor. */
export function NoteEditorSkeleton({
  label = 'Decrypting note',
}: {
  label?: string
}) {
  return (
...
/** The vault gate while we work out whether a vault exists. */
export function VaultGateSkeleton() {
  return (
    <SkeletonScreen label="Opening vault" className="py-12">
      <Skeleton className="h-7 w-48" />
      <SkeletonText lines={2} className="mt-4 max-w-md" />
      <Skeleton className="mt-6 h-11 w-full max-w-sm" />
    </SkeletonScreen>
  )
}
